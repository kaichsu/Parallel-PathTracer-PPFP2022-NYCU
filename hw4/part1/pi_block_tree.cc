...
int main(int argc, char **argv)
{
    ...
    for(int stride = 1; stride < world_size; stride <<= 1){
        if(world_rank % (stride << 1) != 0){
            int dest = world_rank - stride;
            MPI_Send(&total_count, 1, MPI_LONG_LONG, dest, world_rank, MPI_COMM_WORLD);
            break;
        } else {
            long long count = 0;
            int src = world_rank + stride;
            MPI_Status status;
            MPI_Recv(&count, 1, MPI_LONG_LONG, src, src, MPI_COMM_WORLD, &status);
            total_count += count;  
        }
    }
